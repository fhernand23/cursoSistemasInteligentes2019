...
class qlearning_mountaincar():
    ...
    def choose_action(self, state, epsilon):
        cont_action = self.env.action_space.sample() if (np.random.random() <= epsilon) else np.argmax(self.Q[state])
...
qrunner = qlearning_mountaincar(episodes=200)
qrunner.run()
