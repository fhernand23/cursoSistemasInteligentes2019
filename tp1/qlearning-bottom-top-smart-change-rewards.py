...
import numpy as np
import pandas as pd
from gymEnvBottonTop import BottomTopEnv, Position, Action
...
def qLearning(env, num_episodes, discount_factor=1.0,
    ...
    for ith_episode in range(num_episodes):
        # if episode 50 change rewards
        if (ith_episode == 50):
            newShape = defaultdict();
            newShape[Position.BOTTOM] = [[Action.REST, 0.2, Position.BOTTOM], [Action.CLIMB, 0.2, Position.TOP],
                                           [Action.DOWN, -1, Position.BOTTOM]]
            newShape[Position.MIDDLE] = [[Action.REST, -1, Position.MIDDLE], [Action.CLIMB, 0.1, Position.TOP],
                                           [Action.DOWN, 0.6, Position.BOTTOM]]
            newShape[Position.TOP] = [[Action.REST, 0.3, Position.TOP], [Action.CLIMB, -1, Position.TOP],
                                        [Action.DOWN, 0.2, Position.MIDDLE]]
            env.changeRewards(newShape)
