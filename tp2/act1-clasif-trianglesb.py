...
# triangle 2: 0.4 0.6 - 0.5 0.9 - 0.6 0.2
f4 = lambda x: (-2 * x + 1.4)
f5 = lambda x: (3 * x - 0.6)
f6 = lambda x: (-7 * x + 4.4)
...
f4y = [(f4)(x) for x in x1]
f5y = [(f5)(x) for x in x1]
f6y = [(f6)(x) for x in x1]
for i in range(points):
    if ((f1y[i] <= y1[i] and f2y[i] >= y1[i] and f3y[i] <= y1[i]) and not (f4y[i] <= y1[i] and f5y[i] >= y1[i] and f6y[i] >= y1[i])):
        lab1[i] = 1
# generate test
...
for i in range(points_test):
    if ((f1y[i] <= y1_test[i] and f2y[i] >= y1_test[i] and f3y[i] <= y1_test[i]) and not (f4y[i] <= y1_test[i] and f5y[i] >= y1_test[i] and f6y[i] >= y1_test[i])):
        lab1_test[i] = 1
...
def draw_triangles_points():
    ...
    plt.plot(x, f4(x), color='gray', label='triangle 2')
    plt.plot(x, f5(x), color='gray')
    plt.plot(x, f6(x), color='gray')
