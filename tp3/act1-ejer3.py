...
class Bandit:
    ...
    def __init__(self, k_arm=10, epsilon=0., initial=0., step_size=0.1, sample_averages=False, true_reward=0.,
                 epsilon_variable=False, eps_1=0, eps_2=0, eps_3=0, eps_4=0, softmax=False, temperature=40):
        self.k = k_arm
...
def generate_reward_distrib():
    ...
    plt.savefig('./images/fig1_3_reward_distrib.png')
    plt.close()


def run_comparison(runs=2000, time=1000):
    epsilons = ['variable epsilon', 'variable epsilon optimistic', 'softmax', 'softmax optimistic']
    bandits = [Bandit(epsilon=0.1, sample_averages=True, epsilon_variable=True, eps_1=0.1, eps_2=0.05, eps_3=0.01, eps_4=0.005),
               Bandit(epsilon=0.1, sample_averages=True, epsilon_variable=True, eps_1=0.1, eps_2=0.05, eps_3=0.01, eps_4=0.005, initial=0.5),
			   Bandit(epsilon=0.1, sample_averages=True, softmax=True, temperature=40),
			   Bandit(epsilon=0.1, sample_averages=True, softmax=True, temperature=40, initial=0.5)]
    ...
    for eps, counts in zip(epsilons, best_action_counts):
        plt.plot(counts, label=eps)
    plt.xlabel('steps')
    plt.ylabel('% optimal action')
    plt.legend()

    plt.savefig('./images/fig1_3_comparison.png')
    plt.close()
